...
> 增量备份自**上一次全量备份**后往往不止有一个备份文件，这个时候只需要顺序遍历[**增量备份记录文件**](#增量备份文件记录文件)中的**数组**（元素已经按**时间戳升序**排序），得到每一次备份的**备份名**，**重复执行**上面的恢复步骤即可。  

## 增量备份记录文件

上面提到了[回传备份记录到主控端](#send-back-to-backend)，实际上，**增量备份的记录**在**主控端**(Backend)和**实例端**(InsSide)各存留了一份：

- 主控端：`server_data/backup_records.json`

- 实例端：`增量备份目录/backup_records.json`  
